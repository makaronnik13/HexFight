...
public class BattleTrigger : MonoBehaviour {
	...
	{
		Debug.Log ("enter");
		if(InvokeOnTriggerEnter && c.GetComponent<BattleWarrior>() && !c.GetComponent<BattleWarrior>().Enemy)
		{
			StartBattle ();
		}
	}
	...
	public void StartBattle()
	{
		...
		Instantiate (GridBattleField, transform.position, Quaternion.identity).GetComponent<HexField>().GenerateCells(positions, obstaclesLayers, recievingRaycastLayer, Offset.Min, Offset.Max);
	}
	...
	public void Start()
	{
		...
		//GridBattleField.GetComponent<HexField> ().GenerateCells (cellsAims);
	}

	private Vector2 CellCoordToWorld(Vector2 cellCoord)
	{
		float x =  transform.lossyScale.x * CellSize *  (float)Mathf.Sqrt(3) * (cellCoord.x + cellCoord.y/2);
		float y =  transform.lossyScale.x * CellSize * 3 / 2 * -cellCoord.y;
	...
	private bool Raycast(Vector3 fromPosition, float minOffset, float maxOffset, LayerMask obstacleLayer, LayerMask raycastingLayer)
	{
		...
		{
			Collider[] hitColliders = Physics.OverlapCapsule(hit.point+Vector3.down * CellSize*3, hit.point+ Vector3.up * CellSize*3, CellSize*2);
